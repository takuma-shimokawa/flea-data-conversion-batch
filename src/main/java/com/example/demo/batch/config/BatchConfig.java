...
/**
 * 商品データ移行バッチのJob・Stepを定義する設定クラス。
 *
 * categoryStepでカテゴリデータをcategoryテーブルへ登録した後、
 * itemsStepで商品データをitemsテーブルへ移行する。
 *
 * 商品データのうち正常データはitemsテーブルへ登録し、
 * エラーデータはCSVファイルへ出力する。
 */
@Configuration
public class BatchConfig {

    // application.ymlのbatch.chunk-sizeを取得する
    @Value("${batch.chunk-size}")
    private int chunkSize;

    /**
     * 商品データ移行バッチ全体を実行するJobを作成する。
     *
     * categoryStepを先に実行し、正常終了後にitemsStepを実行する。
     *
     * @param jobRepository         Jobの実行履歴や状態を管理するRepository
     * @param categoryStep          カテゴリ移行処理を行うStep
     * @param itemsStep             商品移行処理を行うStep
     * @param executionTimeListener Jobの実行時間を計測するListener
     * @return カテゴリ移行後に商品移行を行うmigrationJob
     */
    ...
    /**
     * originalテーブルのcategory_nameをもとに、
     * categoryテーブルへカテゴリを登録するStepを作成する。
     *
     * Readerで重複しないcategory_nameを取得し、
     * Processorで最大3階層のCategoryへ変換する。
     * Writerでは親カテゴリから順番にcategoryテーブルへ登録する。
     *
     * @param jobRepository         Stepの実行履歴や状態を管理するRepository
     * @param transactionManager    Chunk単位のトランザクションを管理するためのもの
     * @param categoryReader        category_nameを読み込むReader
     * @param categoryProcessor     category_nameを階層ごとのCategoryへ変換するProcessor
     * @param categoryWriter        categoryテーブルへカテゴリを登録するWriter
     * @param progressListener      処理件数を定期的にログ出力するListener
     * @param executionTimeListener Stepの実行時間を計測するListener
     * @return カテゴリ移行処理を行うcategoryStep
     */
    ...
    public Step categoryStep(
        ...
        return new StepBuilder("categoryStep", jobRepository)
                ...
                .<String, List<Category>>chunk(chunkSize, transactionManager)
    ...
    public Step itemsStep(
        ...
        return new StepBuilder("itemsStep", jobRepository)
                ...
                .<Original, ItemProcessResult>chunk(chunkSize, transactionManager)
     ...
     */
    @Bean
    public ClassifierCompositeItemWriter<ItemProcessResult> errorCheck(ItemsWriter itemsWriter,
            ErrorCsvWriter errorCsvWriter) {
